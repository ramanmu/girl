...
#{
    ...
    allowed_filters = [c.strip() for c in cfg.get("filter_columns", "").split(",") if c.strip()]
    
    print(f"--- 🛠️  Generating Artifacts for: {csv_path} ---")
    df = pd.read_csv(csv_path).fillna('')
    
    # 1. SCHEMA GENERATION (Whitelisted Only)
    ...
    #{
        ...
        #{
            if pd.api.types.is_numeric_dtype(df[col]):
                schema["filters"].append({"column": col, "type": "range", "min": float(df[col].min()), "max": float(df[col].max())})
            else:
    ...
    # 3. MANIFEST
    manifest = {"model_name": cfg.get("model_name"), "files": {
