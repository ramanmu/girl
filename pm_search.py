...
#{
    ...
    #{
        ...
        manifest_path = self.cfg.get("manifest_file")
        with open(manifest_path, "r") as f: self.manifest = json.load(f)
        ...
        # Build keyword logic from source text strings
        ...
        self.epsilon = self.cfg.getfloat("epsilon", 1e-09)
        self.rrf_k = 60
    #}
    ...
    #{
        ...
        #{
            ...
            if h.hexdigest() != info["hash"]:
                raise PermissionError(f"CRITICAL: Integrity failure in {info['name']}")
        #}
    #}

    def execute_query(self, dsl):
    #{
        query = dsl.get("nlp", "").strip()
        ...
        if f_df.empty or top_k < 0: return pd.DataFrame()
        if top_k == 0 or not query: return f_df.head(top_k).assign(rrf_score=0.0)

        # 2. Hybrid Search Scores
        subset_ids = f_df.index.tolist()
        q_vec = self.model.encode([query], convert_to_numpy=True)
        faiss.normalize_L2(q_vec)
        
        subset_vecs = np.array([self.vector_index.reconstruct(i) for i in subset_ids])
        v_scores = np.dot(subset_vecs, q_vec.T).flatten()
